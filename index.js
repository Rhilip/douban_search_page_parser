...
K.write = function (t, e, r, n, o, i) {
  var a, s, u, c = 8 * i - o - 1, f = (1 << c) - 1, l = f >> 1, h = 23 === o ? Math.pow(2, -24) - Math.pow(2, -77) : 0,
    p = n ? 0 : i - 1, d = n ? 1 : -1, m = e < 0 || 0 === e && 1 / e < 0 ? 1 : 0;
  for (e = Math.abs(e),
...
function V(t) {  // 9  13  16  25  29  33
  return base64js.toByteArray(q(t))
}
...
function Bt(t) {
  ...
  function h(r) {
    var n = r.offset
      ...
      , i = Math.pow(2, o);
    if (i > 4)
      return u_h_Bt(t.slice(n + 1, n + 1 + i));
    if (i < e.maxObjectSize)
      return a_h_Bt(t.slice(n + 1, n + 1 + i));
...
function c_g_Bt(t) {
  for (let e = t.length, r = 0; r < e; r += 2) {
    let n = t[r];
    t[r] = t[r + 1];
    t[r + 1] = n;
  }
...
function decrypt(r) {
  let a = Buffer.from(r, "base64"); // 0
  let s =  Math.max(Math.floor((a.length - 2 * i) / 3), 0);  // 40
  let u = a.slice(s, s + i);  // 41

  a = Buffer.concat([a.slice(0, s), a.slice(s + i)]);  // 43  45  47

  let c_data = XXH.h64(41405).update(
    Buffer.concat([u, Buffer.from("")])
  ).digest().toString(16);  // 49  67  69
